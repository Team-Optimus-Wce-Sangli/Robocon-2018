...
PID myPID1(&encoderValue1,&pwm_val1,&reqd_pos,Kp,Ki,Kd, DIRECT);
PID myPID2(&encoderValue2,&pwm_val2,&reqd_pos, Kp, Ki, Kd, DIRECT);
PID myPID3(&encoderValue3,&pwm_val3,&reqd_pos,Kp,Ki,Kd,DIRECT);
PID myPID4(&encoderValue4,&pwm_val4,&reqd_pos,Kp,Ki,Kd,DIRECT);
void setup()
{
...
void forward2()
{ 
    analogWrite(pwm1,sp12);//m1
    analogWrite(pwm2,sp22);//m2
    analogWrite(pwm3,sp32);//m3
    analogWrite(pwm4,sp42);//m4
    digitalWrite(dir1,0);//m1
    digitalWrite(dir2,1);//m2
    digitalWrite(dir3,1);//m3
    digitalWrite(dir4,0);//m4
...
//if(s==1)
//{
//  for(int tim=millis();millis()<tim+1000;)
//  {
//    myPID1.Compute();
//    setPWM1(pwm_val1);
//      myPID2.Compute();
...
void loop() {
    ...
    //Serial.println(encoderValue1);Serial.print(",");Serial.print(encoderValue2);Serial.print(",");Serial.print(encoderValue3);Serial.print(",");Serial.print(encoderValue4);
      ...
      {
        leftturn1(); i=0;j=0;s=1;
      }
      else if(c=='d'&m==1)
      {
        rightturn1(); i=0;j=0;s=1;
      }
       if(c=='e'&m==2)
      {
       forward2();
      ...
      {
        leftturn2(); i=0;j=0;s=0;
      }
      else if(c=='d'&m==2)
      {
        rightturn2(); i=0;j=0;s=0;
      }
        else if(c=='i')
      {
        up(); i=0;j=0;s=0;
